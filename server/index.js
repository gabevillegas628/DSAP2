...
app.options('*', cors({
  origin: ['http://localhost:3000', 'http://localhost:3001', 'https://cd3c2444922a.ngrok.app', 'https://7c0c7e8b626b.ngrok.app'],
...
const storage = multerS3({
  ...
  key: function (req, file, cb) {
    // Keep your existing filename logic
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
    cb(null, uniqueSuffix + '-' + file.originalname);
  }
});
...
// UPDATE your delete endpoint to also remove from S3:
...
app.get('/api/practice-clones/:id/download', async (req, res) => {
  try {
    ...
    const params = {
      ...
      Expires: 3600,
      ResponseContentDisposition: `attachment; filename="${practiceClone.originalName}"`
    };
